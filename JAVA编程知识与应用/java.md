...
1. 单机MySQL时代：
   ...
   <img src="imgs/image-20211001100923197.png" alt="image-20211001100923197" style="zoom:80%;" />

   90年代，一个基本网站访问量不大，单个数据库完全足够

   那个时候，更多使用静态网页HTML，服务没器没有压力
...
2. Memcached(缓存)+MySQL+垂直拆分(读写分离)
   ...
   网站80%的情况都是在读，每次都去查询数据库的话就十分的麻烦。所以说我们可以使用缓存减轻数据库的压力。

   <img src="imgs/image-20211001101522033.png" alt="image-20211001101522033" style="zoom:80%;" />

   有三个数据库，其中一个只负责写，另外两个负责读，写完后会将数据同步到读数据库中，并且应用使用cache缓存进行数据的读写操作。

   发展过程：优化数据结构和索引----->文件缓存--->Memcached

3. 分库分表+水平拆分+MySQL集群

   将数据拆分三份，分别放在不同的集群节点上。比如不同的数据库放不同的节点，不同的表放在不同的节点。

   <img src="imgs/image-20211001102806928.png" alt="image-20211001102806928" style="zoom:80%;" />

4. 最近的年代

   MySql等关系型数据库不够用了，数据量多，变化很快。

   mysql有的使用它来存放一些比较大的文件，博客，图片。数据库很大效率很低，如果有一种数据库专门处理这种数据，mysql压力就变得十分小。图数据库，文件数据库等。

   <img src="imgs/image-20211001105006176.png" alt="image-20211001105006176" style="zoom:80%;" />

5. 为什么要用NOSQL？

   用户的个人信息，社交网络，地理位置，用户自己产生的数据，用户日志等等爆发性增长。

   这时候就可以使用NOSQL数据库，可以很好处理以上情况。
...
* 解藕
* 方面扩展，（数据之间没有关系了，很好扩展）
* 大数据量高性能，（redis一秒写8万次，读11万次，NOSQL的缓存记录级的，是一种细粒度缓存，性能比较高）
